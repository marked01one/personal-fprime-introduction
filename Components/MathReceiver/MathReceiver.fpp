module MathModule {
    ...
    queued component MathReceiver {
        ...
        sync input port scheduleIn: Svc.Sched
        ...
        param FACTOR: F32 default 1.0 \
            id 0 set opcode 10 save opcode 11
        ...
        ) severity activity high id 0 format "Factor updated to {f}" throttle 3

        @ Math operation performed
        event OPERATION_PERFORMED(
            val: MathOp
        ) severity activity high id 1 format "{} operation performed"

        @ Event throttle cleared
        event THROTTLE_CLEARED \
            severity activity high id 2 format "Event throttle cleared"
